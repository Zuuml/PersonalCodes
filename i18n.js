(() => {
const T={
ru:{code:'RU',currency:'RUB',locale:'ru-RU',rate:1,round:1000,
...
en:{code:'EN',currency:'USD',locale:'en-US',rate:.012,round:10,
...
ja:{code:'日本語',currency:'JPY',locale:'ja-JP',rate:1.75,round:1000,
...
zh:{code:'中文',currency:'CNY',locale:'zh-CN',rate:.086,round:10,
...
const region={ru:{currency:'RUB',locale:'ru-RU',rate:1,round:1000},en:{currency:'USD',locale:'en-US',rate:.012,round:10},ja:{currency:'JPY',locale:'ja-JP',rate:1.75,round:1000},zh:{currency:'CNY',locale:'zh-CN',rate:.086,round:10}};
function detect(){const s=localStorage.getItem('forged-lang');if(s&&T[s])return s;const l=(navigator.language||'ru').toLowerCase();if(l.startsWith('ja'))return'ja';if(l.startsWith('zh'))return'zh';if(l.startsWith('en'))return'en';return'ru'}
function price(v,lang,withFrom=true){const c=region[lang],t=T[lang];const n=Math.round((v*c.rate)/c.round)*c.round;let f=new Intl.NumberFormat(c.locale,{style:'currency',currency:c.currency,maximumFractionDigits:0}).format(n);return (withFrom?t.from+' ':'')+f}
function apply(lang){const t=T[lang]||T.ru;document.documentElement.lang=lang==='zh'?'zh-CN':lang;document.querySelectorAll('[data-i18n]').forEach(e=>{let k=e.dataset.i18n;if(t[k]!=null)e.textContent=t[k]});document.querySelectorAll('[data-price]').forEach(e=>e.textContent=price(+e.dataset.price,lang,true));document.querySelectorAll('[data-locale-label]').forEach(e=>e.textContent=t.code+' · '+region[lang].currency);document.querySelectorAll('[data-lang]').forEach(e=>e.classList.toggle('active',e.dataset.lang===lang));localStorage.setItem('forged-lang',lang);window.dispatchEvent(new CustomEvent('langchange',{detail:{lang,t}}))}
function setup(){document.querySelectorAll('.locale-btn').forEach(b=>b.onclick=e=>{e.stopPropagation();b.closest('.locale').classList.toggle('open')});document.querySelectorAll('[data-lang]').forEach(b=>b.onclick=()=>{apply(b.dataset.lang);document.querySelectorAll('.locale').forEach(x=>x.classList.remove('open'))});document.addEventListener('click',()=>document.querySelectorAll('.locale').forEach(x=>x.classList.remove('open')));apply(detect())}
window.FI18N={T,region,price,apply,detect};document.addEventListener('DOMContentLoaded',setup)
})();